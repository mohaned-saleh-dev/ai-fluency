...
8. [Knowledge Base Enhancement](#8--knowledge-base-enhancement)
9. [Partner Identification & Routing](#9--partner-identification--routing)
10. [Handover Scenarios](#10--handover-scenarios)
11. [Technical Requirements](#11--technical-requirements)
12. [Rollout Plan](#12--rollout-plan)
13. [Dependencies & Risks](#13-%EF%B8%8F-dependencies--risks)
...
### 5.1 Pre-Onboarding Partner (New Persona for Chatbot)
...
### 5.2 Live Partner (Existing Persona)
...
### 7.1 Orchestrator Modifications

#### 7.1.1 Partner Status Detection
...
#### 7.1.2 Onboarding Chatbot Instructions

New orchestrator instructions for onboarding partners:
...
#### 7.2.2 Knowledge Structure in MongoDB

```javascript
// New document schema for onboarding knowledge
{
  "_id": ObjectId,
  ...
  "metadata": {
    ...
    "applicable_platforms": [String], // Shopify, Magento, WooCommerce, Custom
    "complexity": "beginner" | "intermediate" | "advanced"
  }
}
```

#### 7.2.3 Content Categories

| Category | Source | Examples |
|----------|--------|----------|
| **API Documentation** | docs.tamara.co | Authentication, endpoints, request/response formats |
| **Integration Guides** | docs.tamara.co | Shopify, Magento, WooCommerce, custom integration |
| **Webhook Setup** | docs.tamara.co | Webhook configuration, events, testing |
| **Sandbox Testing** | docs.tamara.co | Sandbox environment, test credentials, go-live process |
| **Application Process** | SOPs | Steps, timeline, requirements by country |
| **Document Requirements** | SOPs | CR, VAT, bank details, identity documents by entity type |
| **Common Rejection Reasons** | SOPs | Incomplete docs, compliance issues, business type restrictions |
| **Onboarding FAQs** | SOPs | Timeline, process questions, status inquiries |

### 7.3 Onboarding-Specific Intents

#### 7.3.1 New/Enhanced Intents
...
### 7.4 Partner Identification Logic

#### 7.4.1 Salesforce Configuration Fields
...
#### 7.4.2 Classification Logic

```python
def classify_partner(salesforce_account):
    """
    Classify partner as 'onboarding' or 'live' based on Salesforce config.
    """
...
### 8.1 docs.tamara.co Content Ingestion

#### 8.1.1 Content Sources
...
### 8.2 Partner Onboarding SOPs

#### 8.2.1 Content Categories
...
#### 8.2.2 SOP Document Format

```markdown
# SOP: [Topic Name]

## Overview
[Brief description of the process/requirement]

## Applicable To
- Countries: [KSA, UAE, etc.]
- Entity Types: [All, Sole Proprietor, LLC, etc.]
- Onboarding Stage: [Application, Documents, Integration, Approval]
...
## Related Links
- [Links to docs.tamara.co or other resources]
```

### 8.3 Knowledge Retrieval Enhancement
...
flowchart TD
    ...
    B -->|Technical/Integration| C[Search docs.tamara.co]
    B -->|Process/Requirements| D[Search SOPs]
    B -->|General FAQ| E[Search support.tamara.co]
    B -->|Mixed/Unclear| F[Search All Sources]
    
    C --> G[Rank by Relevance]
    D --> G
    E --> G
    F --> G
    
    G --> H{Top Result Score > Threshold?}
    H -->|Yes| I[Return Response]
    H -->|No| J[Fallback to General Response + Offer Handoff]
```

#### 8.3.2 Source Priority for Onboarding Partners

| Query Category | Primary Source | Secondary Source |
|----------------|----------------|------------------|
| API/Integration | docs.tamara.co | Partner Onboarding SOPs |
| Documents/Requirements | Partner Onboarding SOPs | support.tamara.co |
| Application Status | Partner Onboarding SOPs | (API lookup if available) |
| Webhooks/Testing | docs.tamara.co | Partner Onboarding SOPs |
| General FAQs | support.tamara.co | Partner Onboarding SOPs |

---

## 9. 🔄 Partner Identification & Routing

### 9.1 Session Initialization Flow
...
sequenceDiagram
    ...
    Partner->>ChatWidget: Asks Question
    ChatWidget->>Orchestrator: Process Query
    Orchestrator->>KnowledgeBase: Retrieve Relevant Content
    KnowledgeBase-->>Orchestrator: Top Results
    Orchestrator-->>ChatWidget: Generate Response
    ChatWidget-->>Partner: Display Answer
```

### 9.2 Partner Status Caching

To avoid repeated Salesforce lookups:

| Field | TTL | Refresh Trigger |
|-------|-----|-----------------|
| `partner_status` | 24 hours | Session start, explicit refresh |
| `onboarding_stage` | 1 hour | Session start, handover |
| `merchant_id` | 24 hours | Session start |
...
## 10. 🔀 Handover Scenarios

### 10.1 Onboarding Partner Handover Triggers
...
### 10.4 Handover Message Templates

**English:**
```
I'll connect you with our Partner Onboarding team who can help you further with this. 
Please hold while I transfer you – they'll have all the context from our conversation.
```
...
### 12.3 Success Criteria for Each Phase
...
- [Partner AI Chatbot PRD (Original)](link)
- [Salesforce Integration Documentation](link)
- [Knowledge Base Architecture](link)
- [docs.tamara.co Content Index](link)
